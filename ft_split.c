...
/*   Updated: 2023/11/06 21:34:04 by qvan-ste      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
char	**free_arr(char **split_arr, int n)
{
	while (n >= 0)
	{
		free(split_arr[n]);
...
void	init_vars(int *i, int *start, int *n)
{
	*i = 0;
	*start = 0;
	*n = 0;
...
void	skip_i(char const *s, int *i, char c)
{
	while (s[*i] && s[*i] != c)
		*i = *i + 1;
}
